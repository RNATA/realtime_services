class Provider < ApplicationRecord
  ...
  def self.find_active(coords, category)
    ...
    active_providers.map {|ps| {:id => ps.id, :name => ps.provider.first_name, :base_rate => ps.base_rate, :phone_number => ps.provider.phone_number, :rating => ps.provider.rating, :company_name => ps.provider.company_name, :lat => ps.provider.lat, :long => ps.provider.long } }
  end
